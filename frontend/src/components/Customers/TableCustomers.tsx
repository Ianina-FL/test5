...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/customers/customersSlice';
...
const TableSampleCustomers = ({
  ...
  useEffect(() => {
    if (customersNotify.showNotification) {
      notify(
        customersNotify.typeNotification,
        customersNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/customers/customers-view/?id=${id}`);
  };
